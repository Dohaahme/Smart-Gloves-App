...
//   final bool isAdmin;
//   final bool isLogin;
//   const VerbsPage({Key? key, required this.isAdmin,required this.isLogin,}) : super(key: key);
//
//   @override
...
//   List<bool> _isFavoritedInitialState = [];
...
//                   Navigator.pop(context);
//                   Navigator.push(context, MaterialPageRoute(builder: (context) => VerbsPage(isAdmin: widget.isAdmin,isLogin: widget.isLogin,)));
//                 },
...
//     final user = FirebaseAuth.instance.currentUser;
//     String? userId = user?.uid;
//     String cardId = _cards[index]['word_en']; // Assuming word_en is unique
//
...
//           .where('word_en', isEqualTo: cardId)
//           .get()
//           .then((querySnapshot) {
//         querySnapshot.docs.forEach((doc) {
...
//       _prefs?.setBool('favorite_$cardId', false);
//     } else {
...
//
//         // Save state in SharedPreferences
//         _prefs?.setBool('favorite_$cardId', true);
//       } else {
//         print('Word already exists in favorites.');
//       }
//     }
//   }
//
...
//                         // alert2(context, languageProvider
//                         //     .localizedStrings['loadyourwords'] ??
//                         //     'Load Your Words', languageProvider
//                         //     .localizedStrings['errorload'] ??
//                         //     "Sorry Can't Load Your Words Know");
...
//                               IconButton(
//                                 icon: Icon(
//                                   _isFavorited[index] ? Icons.favorite : Icons
//                                       .favorite_border,
...
class _VerbsPageState extends State<VerbsPage> {
  List<Map<String, dynamic>> _cards = [];
  final FlutterTts flutterTts = FlutterTts();
  final DatabaseReference _messageRef = FirebaseDatabase.instance.reference()
      .child('word');
  final DatabaseReference _messagecheck = FirebaseDatabase.instance.reference()
      .child('check');
  final DatabaseReference _messagecheck_add = FirebaseDatabase.instance
      .reference().child('check_add');
  bool network = false;
  List<bool> _isFavorited = [];
  List<bool> _isFavoritedInitialState = [];
  SharedPreferences? _prefs;
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: [
            TextButton(
              child: Text(languageProvider.localizedStrings['ok'] ?? 'Ok',
                  style: TextStyle(color: Colors.blue)),
              onPressed: () {
                Navigator.pop(context);
              },
  ...
  Future<void> _loadCards() async {
    ...
    FirebaseFirestore.instance
        .collection('users')
        .doc("SnGEBUjhAdbtflEYzF1C3JgXBD33")
        .collection('verbs')
        .get()
        .then((querySnapshot) {
      querySnapshot.docs.forEach((doc) {
  ...
  Future<void> _loadFavoriteStates() async {
    ...
    for (int i = 0; i < _cards.length; i++) {
      String cardKey = 'favorite_${_cards[i]['word_en']}';
      bool isFavorited = prefs.getBool(cardKey) ?? false;
      setState(() {
